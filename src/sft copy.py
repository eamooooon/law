...
支持的数据格式（JSONL）：
    ...
    instruction、input、reasoning、output 均为必填。
...
def _collect_data_files(file_dir: str):
    """收集目录或单文件的 json/jsonl 路径列表。"""
    if os.path.isfile(file_dir):
        return [file_dir]
    return glob(f"{file_dir}/**/*.json", recursive=True) + glob(
        f"{file_dir}/**/*.jsonl", recursive=True
    )
...
def main():
    ...
    def preprocess_function(examples):
        ...
        if (
            ...
            or inputs is None
            or reasonings is None
            or outputs is None
        ):
            raise ValueError(
                "数据集必须包含且提供以下列：'instruction'、'input'、'reasoning'、'output'。"
        ...
        for i, (instruction, input_text, reasoning_text, output_text) in enumerate(
            ...
            if not instruction or not input_text or not reasoning_text or not output_text:
                raise ValueError(
                    "发现缺失必填字段的样本：instruction/input/reasoning/output 均不能为空。"
                )

            # 训练目标固定为 <think>reasoning</think> + output；output_model 暂不使用。
            assistant_text = f"{_wrap_with_think_tag(reasoning_text)}\n{output_text}"
