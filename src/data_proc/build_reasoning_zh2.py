#!/usr/bin/env python3
"""
Generate missing reasoning/output_model for Chinese legal multiple-choice JSONL datasets.

Key behaviors:
- First-person Chinese reasoning generation
- Append/update output_model field
- Resume support
- Fail-fast on generation failure (stop immediately, do not write failed record)
- Optional concurrent API generation with ordered writes
"""
...
DEFAULT_INPUT_PATH = os.path.join(PROJECT_ROOT, "datasets", "proc", "jec_qa_5000.jsonl")
DEFAULT_OUTPUT_PATH = os.path.join(PROJECT_ROOT, "datasets", "proc")
...
SYSTEM_PROMPT = (
    "你是一名法律考试解析员，正在构建中文法律选择题训练数据。"
    "你必须始终使用第一人称进行分析，不得使用第三视角自称（如‘模型认为’‘助手认为’）。"
    "你的分析必须围绕题干与选项展开，避免空泛套话和模板化表述。"
)

PROMPT_TEMPLATE = """你将收到一条中文法律选择题样本，包含 instruction、input、gold_output。

任务要求：
1) 生成 reasoning：用第一人称写出我如何抓住题干关键点、比对各选项并确定正确答案。
2) 生成 output_model：只输出最终选项字母，多个答案用英文逗号分隔（例如 A,C 或 B,D）。
3) 不要暴露与任务无关的隐式思维链，只输出可用于训练的数据化推理文本。
4) 不得出现“作为AI/模型”等第三视角自称。
5) output_model 必须与 gold_output 完全一致（字母大写、逗号分隔、不加空格和解释）。

质量要求：
- reasoning 必须点出题干中的法律要点，并说明关键选项的取舍依据。
- reasoning 重点解释“为什么这些选项正确、为什么其他选项不成立或不优先”。
- reasoning 不要逐字复述题干原文。

风格约束：
- reasoning 为一段中文第一人称分析，重依据和可读性，但不得超过 1000 字。
- output_model 仅保留字母答案，不得添加任何中文或英文说明。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build missing reasoning/output_model for Chinese JSONL with LLM.")
    parser.add_argument(
        "--input-path",
        type=str,
...
def process_file(args: argparse.Namespace, input_file: str, output_file: str) -> Dict[str, int]:
    ...
    file_total_rows = count_nonempty_lines(input_file)
