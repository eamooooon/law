#!/usr/bin/env python3
"""
Generate missing reasoning/output_model for Chinese legal-summary style JSONL datasets.
...
DEFAULT_INPUT_PATH = os.path.join(PROJECT_ROOT, "datasets", "proc", "cail2022_sfzy_sft.jsonl")
...
SYSTEM_PROMPT = (
    "你是一名法律文本分析员，正在构建中文司法摘要训练数据。"
    "你必须始终使用第一人称进行分析，不得使用第三视角自称（如‘模型认为’‘助手认为’）。"
    "你的分析必须紧扣输入事实，避免空泛套话和模板化表述。"
)

PROMPT_TEMPLATE = """你将收到一条中文司法文本样本，包含 instruction、input、gold_output。

任务要求：
1) 生成 reasoning：用第一人称写出我如何提炼事实主线、识别争议焦点、筛除冗余信息并形成摘要思路。
2) 生成 output_model：用第一人称给出最终摘要结果（简洁、信息密度高、与事实一致）。
3) 不要暴露与任务无关的隐式思维链，只输出可用于训练的数据化推理文本。
4) 不得出现“作为AI/模型”等第三视角自称。

质量要求：
- reasoning 必须引用输入中的关键事实锚点（人物、时间、行为、程序进展、裁判/处理结果等）。
- reasoning 要解释“为什么保留这些信息、为什么舍弃冗余信息”。
- output_model 要与 gold_output 的事实方向一致，但不能机械照抄。

风格约束：
- reasoning 为一段中文第一人称分析，重深度不重凑字数，但不得超过 1000 字。
- output_model 为中文第一人称简明摘要。
